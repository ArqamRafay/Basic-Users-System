 ...
 */
 function usePasswordGrant (done) {
   var details = this.req.body;
   console.log("you are in usePasswordGrant function to check.........");
   // User credentials

   if ((!details.email || !details.password) && (!details.email || !details.fb_id) && (!details.email || !details.google_id)) {
     return done(error('invalid_client',
       'Missing parameters. "username" and "password" are required'));
   }

   var self = this;
   return this.model.getUser(this.req.body, function (err, user) {
     if (err) return done(error('server_error', false, err));
     if (!user) {
...
function useRefreshTokenGrant (done) {
  ...
  this.model.getRefreshToken(token, function (err, refreshToken) {
    console.log('HEREEEEEEEEEEEEEEE : ', refreshToken);
    // console.log('HEREEEEEEEEEEEEEEE SELFFFFF : ', self);
    if (err) return done(error('server_error', false, err));
    ...
    if (!refreshToken.user && !refreshToken.user_id) {
      return done(error('server_error', false,
